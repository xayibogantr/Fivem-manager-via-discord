...
try {
    ...
    if (z.QBCore) z.utils.log.info("xayibogantr - Bot baslatiliyor.");
...
for (const file of addons) {
    try {
        ...
        z.utils.log.info(`[xayibogantr] ${file} bot baslatiliyor.`);
    } catch (e) {
        z.utils.log.error(`[xayibogantr] ${file} bot baslatilamadi.`);
        z.utils.log.error(e);
    }
}


SetConvarReplicated("xayibogantr_servername", z.config.FiveMServerName);
SetConvarReplicated("xayibogantr_discordinvite", z.config.DiscordInviteLink);
SetConvarReplicated("xayibogantr_serverip", z.config.FiveMServerIP);
SetConvarReplicated("xayibogantr_userpresence", String(z.config.enableUserPresence));
...
on("playerJoining", (oldId) => {
    ...
    if (z.bot.isRolePresent(member, z.config.StaffChatRoleIds)) {
        ExecuteCommand(`add_principal "player.${source}" group.xayibogantrstaff`);
    }
});
...
if (z.config.EnableStaffChatForwarding) {
    RegisterCommand("staff", (source, args, raw) => {
        if (!IsPlayerAceAllowed(source, "xayibogantr.staffchat")) return;
        ...
        staffChannel.send({ content: `${GetPlayerName(source)}: ${raw.substring(6)}`, allowMentions: false }).catch((e) => {
            z.utils.log.error("xayibogantr iletisime gec. server error about. krds.");
        });
    }, false);

    RegisterCommand("stafftoggle", (source, args, raw) => {
        if (IsPlayerAceAllowed(source, "xayibogantr.staffchat")) {
            ExecuteCommand(`remove_principal "player.${source}" group.xayibogantrstaff`);
